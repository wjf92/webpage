    ...
    <div>
      <transition-group name="taglist-moving-animation">
        <Tag type="dot" v-for="item in pageTagsList" :key="item.name" :name="item.name" @on-close="closePage"
             @click.native="linkTo(item.name, item.title, item.path)" :closable="item.name==='home_index'?false:true"
...
<script>
  export default {
    ...
    methods: {
      ...
      linkTo (name, title, path) {
        if (title.substr(0, 5) === 'SQL查询') {
          this.$router.push({
            path
          })
        } else {
    ...
    watch: {
      '$route' (to) {
        this.currentPageName = to.name === 'serach-sql' ? `${to.name}${to.params.id}` : to.name
      }
    }
  }
</script>
